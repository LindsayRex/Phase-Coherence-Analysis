    ...
    for i, chunk in enumerate(chunks):
        ...
        if len(chunk) > 0 and np.all(np.isfinite(chunk)):
            ...
            if chunk_rms > 1e-12:
             ...
             print(f"Warning: Chunk {i} contains non-finite values before norm. Zeroing.")
...
def stitch_signal(chunks, sdr_rate, recon_rate, overlap_factor, tuning_delay, window_type):
    ...
    if not chunks:
        print("Error: No chunks provided for stitching.")
        return None, None
    ...
    if len(chunks[0]) > 0 and recon_rate > 0: # Estimate from first chunk
        chunk_duration_s = len(chunks[0]) / recon_rate
         ...
         print("Warning: Estimating chunk duration from SDR rate for stitching.")
    ...
    if chunk_duration_s <= 0:
         print("Error: Cannot determine chunk duration for stitching buffer calculation.")
         return None, None
    ...
    if num_samples_recon <= 0:
        print(f"Error: Invalid calculated reconstruction samples ({num_samples_recon}).")
        return None, None

    reconstructed_signal = np.zeros(num_samples_recon, dtype=complex)
    sum_of_windows = np.zeros(num_samples_recon, dtype=float)

    print(f"Reconstruction target buffer: {num_samples_recon} samples @ {recon_rate/1e6:.2f} MHz (Est. Duration: {total_duration_recon*1e6:.1f} us)")
    print(f"Using stitching window: '{window_type}' with effective overlap factor: {overlap_factor:.2f}")
    ...
    for i, chunk_to_add in tqdm(enumerate(chunks), total=len(chunks), desc="Stitching"):
        ...
        if actual_len >= 2:
            ...
            except: window = np.ones(actual_len); print(f"Warn: Failed get_window '{window_type}'")
        ...
        try:
            ...
            if not np.all(np.isfinite(segment_data)):
                segment_data = np.nan_to_num(segment_data); print(f"Warn: NaN in chunk {i} stitch")
            ...
            if not np.all(np.isfinite(reconstructed_signal[start_idx_recon:end_idx_recon])):
                ...
                print(f"Warn: NaN after adding chunk {i}")
...
def normalize_stitched_signal(raw_signal, sum_windows, target_rms):
    ...
    if raw_signal is None or sum_windows is None or len(raw_signal) != len(sum_windows) or len(raw_signal) == 0:
         print("Error: Invalid input for stitched signal normalization.")
         return None
    ...
    print(f"  Sum of windows stats: Min={np.min(sum_windows):.4f}, Max={np.max(sum_windows):.4f}, Mean={np.mean(sum_windows):.4f}, Median={np.median(sum_windows):.4f}")
    ...
    print(f"RMS after Sum-of-Windows Norm: {rms_after_sow:.4e}")

    final_signal = normalized_signal.copy()
    if rms_after_sow > 1e-12:
        scale_factor = target_rms / rms_after_sow
        final_signal *= scale_factor
        print(f"Applied final scaling factor {scale_factor:.4f} to match target RMS ({target_rms:.4e}).")
    else: print("Warning: RMS near zero after SoW norm. Skipping final RMS scaling.")
