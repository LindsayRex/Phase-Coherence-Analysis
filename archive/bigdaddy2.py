...
import sys
import pywt  # Add PyWavelets for SWT
...
# --- 3. Phase Alignment Using Stationary Wavelet Transform (SWT) ---
print("\n--- Performing SWT Decomposition and Phase Alignment ---")
wavelet = 'db4'  # Daubechies wavelet
level = 4  # SWT decomposition level
chunk_duration_s = len(loaded_chunks[0]) / GLBL_SDR_SAMPLE_RATE if GLBL_SDR_SAMPLE_RATE > 0 else 0
overlap_samples = int(round(chunk_duration_s * GLBL_OVERLAP_FACTOR * GLBL_RECON_SAMPLE_RATE))
if overlap_samples < 1:
    overlap_samples = 1
    print("Warning: Overlap samples too small. Setting to 1.")

# Decompose each chunk using SWT
swt_coeffs = []
for chunk in upsampled_chunks:
    if len(chunk) == 0:
        swt_coeffs.append(None)
        continue
    # Pad chunk to nearest power of 2 for SWT (if necessary)
    n = len(chunk)
    n_padded = 2**int(np.ceil(np.log2(n)))
    chunk_padded = np.pad(chunk, (0, n_padded - n), mode='constant')
    coeffs = pywt.swt(chunk_padded, wavelet, level=level, start_level=0)
    swt_coeffs.append(coeffs)  # List of (cA, cD) tuples for each level

# Phase alignment using SWT coefficients
phase_corrected_swt_coeffs = [swt_coeffs[0]] if swt_coeffs[0] is not None else [None]
for i in range(1, len(swt_coeffs)):
    if swt_coeffs[i] is None or swt_coeffs[i-1] is None:
        phase_corrected_swt_coeffs.append(swt_coeffs[i])
        continue
    # Align coefficients by minimizing phase discontinuity in overlapping regions
    phase_offsets = []
    for lvl in range(level):
        cA1 = swt_coeffs[i-1][lvl][0][-overlap_samples:]  # Approximation coefficients of previous chunk
        cA2 = swt_coeffs[i][lvl][0][:overlap_samples]     # Approximation coefficients of current chunk
        corr = np.sum(cA1 * np.conj(cA2))
        phase_offset = np.angle(corr)
        phase_offsets.append(phase_offset)
    # Apply phase correction to all coefficients at this level
    corrected_coeffs = []
    for lvl, (cA, cD) in enumerate(swt_coeffs[i]):
        phase_offset = phase_offsets[lvl]
        corrected_cA = cA * np.exp(-1j * phase_offset)
        corrected_cD = cD * np.exp(-1j * phase_offset)
        corrected_coeffs.append((corrected_cA, corrected_cD))
    phase_corrected_swt_coeffs.append(corrected_coeffs)

# Reconstruct chunks using inverse SWT
phase_corrected_chunks = []
for i, coeffs in enumerate(phase_corrected_swt_coeffs):
    if coeffs is None:
        ...
        continue
    reconstructed_chunk = pywt.iswt(coeffs, wavelet)
...
print("--- SWT Phase Alignment Complete ---")
