...
print(f"SNR per Chunk: {snr_db} dB (Relative to signal power *after* processing)")
...
if modulation.lower() == 'qam16':
    ...
    if normalized_bw_cutoff >= 1.0:
        normalized_bw_cutoff = 0.999
...
if modulation.lower() not in ['noise']:
    if generated_baseband is None:
        raise RuntimeError("Baseband signal generation failed.")
...
if gt_rms_before_norm > 1e-20:
    baseband_signal_complex_ground_truth /= gt_rms_before_norm
else:
    print("Warning: Ground truth signal power is near zero. Skipping normalization.")
...
for i in tqdm(range(num_chunks)):
    ...
    filter_order_bpf = 8  # For Cheby2 order 4
    decimation_filter_length_approx = 60 * decimation_factor
    filter_buffer_samples = filter_order_bpf * 5 + decimation_filter_length_approx
    ...
    if start_sample_gt >= actual_end_sample_gt:
        break
    ...
    if time_slice_gt.shape != baseband_slice_gt.shape:
        raise RuntimeError("Shape mismatch after slicing!")
    min_len_for_processing = filter_buffer_samples
    ...
    if i == 0:
        print(f"\n--- Debug: Chunk {i} Processing Stages (Cheby2 Filters) ---")
        print_stats("Baseband Slice", baseband_slice_gt)

    # --- Downconversion ---
    if_signal_chunk_gt_rate = baseband_slice_gt * np.exp(1j * 2 * np.pi * current_if_center * time_slice_gt)
    if i == 0:
        print_stats("IF Signal (GT Rate)", if_signal_chunk_gt_rate)

    # --- Band-Pass Filter (Cheby2) ---
    low_cutoff = current_if_center - sdr_ibw / 2 * 1.1  # Slightly wider to avoid edge effects
    high_cutoff = current_if_center + sdr_ibw / 2 * 1.1
    if low_cutoff <= 0:
        low_cutoff = 1e3
    ...
    if wn_low <= 0 or wn_high >= 1.0:
        if wn_high >= 1.0 and np.isclose(wn_high, 1.0):
            wn_high = 0.999999
        else:
            raise ValueError(f"Chunk {i}: Invalid filter freqs. Wn_low={wn_low:.4f}, Wn_high={wn_high:.4f}.")
    b_bpf, a_bpf = sig.cheby2(4, 60, [wn_low, wn_high], btype='bandpass', analog=False)
    if_signal_chunk_filtered_gt_rate = sig.lfilter(b_bpf, a_bpf, if_signal_chunk_gt_rate)
    # Normalize to maintain RMS
    rms_before_bpf = np.sqrt(np.mean(np.abs(if_signal_chunk_gt_rate)**2))
    rms_after_bpf = np.sqrt(np.mean(np.abs(if_signal_chunk_filtered_gt_rate)**2))
    if rms_after_bpf > 1e-12:
        if_signal_chunk_filtered_gt_rate *= rms_before_bpf / rms_after_bpf
    if i == 0:
        print_stats("After BPF (Cheby2)", if_signal_chunk_filtered_gt_rate)

    # --- Decimate (FIR) ---
    try:
        if_signal_chunk_sdr_rate = sig.decimate(
            if_signal_chunk_filtered_gt_rate,
            decimation_factor,
            n=31,
            ftype='fir',
            zero_phase=False
        ...
        # Normalize to maintain RMS
        rms_after_decimate = np.sqrt(np.mean(np.abs(if_signal_chunk_sdr_rate)**2))
        if rms_after_decimate > 1e-12:
            if_signal_chunk_sdr_rate *= rms_before_bpf / rms_after_decimate
    except ValueError as e:
        print(f"\nError during decimate for chunk {i}: {e}. Input length: {len(if_signal_chunk_filtered_gt_rate)}. Skipping chunk.")
        current_time_offset += duration_per_chunk + tuning_delay
        continue
    if i == 0:
        print_stats("After Decimate", if_signal_chunk_sdr_rate)
    ...
    if i == 0:
        print_stats("After Trim/Pad", if_signal_chunk_sdr_rate)

    # --- Add Noise ---
    signal_power_chunk = np.mean(np.abs(if_signal_chunk_sdr_rate) ** 2)
    if signal_power_chunk < 1e-30:
        noise_chunk = np.zeros(num_samples_per_chunk, dtype=complex)
        if i == 0:
            print("Warning: Signal power near zero before AWGN.")
    else:
    ...
    if i == 0:
        print_stats("After AWGN", if_signal_noisy_chunk)
    ...
    last_chunk_phase_offset = total_phase_offset
    if i == 0:
        print_stats("After Phase Noise", if_signal_noisy_chunk)
    ...
    if max_abs_val_chunk < 1e-15:
        max_abs_val_chunk = 1e-15
    ...
    meta = {
        ...
        'filter_bpf_method': 'cheby2_lfilter',
        'filter_decim_method': 'fir_decimate'
    }
...
output_filename = f"simulated_chunks_{f_rf_center / 1e9:.0f}GHz_{bw_total_signal / 1e6:.0f}MHzBW_{modulation}_sdr{sdr_ibw / 1e6:.0f}MHz_realistic_filt.h5"
...
try:
    with h5py.File(output_filename, 'w') as f:
        ...
        f.attrs['filter_description'] = "Causal BPF (Cheby2 Ord4 rs60) and Causal Decimation (FIR n=31 zero_phase=False)"
...
if plot_results and captured_chunks_data:
    ...
    all_finite = True
    for i_chunk, chunk_data in enumerate(captured_chunks_data):
        if chunk_data is None or len(chunk_data) < 2:
            continue
        ...
        if current_max > max_overall_magnitude:
            max_overall_magnitude = current_max

    if not all_finite:
        print("Skipping spectrum plot due to non-finite data.")
    elif max_overall_magnitude < 1e-12:
        max_overall_magnitude = 1
